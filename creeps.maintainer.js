var creepsMaintainer = {
    run: function() {
        ...
        if (harvesters.length < 4 && Game.spawns['Spawn1'].canCreateCreep(standardBody) == OK) {
            var spawnedCreep = Game.spawns['Spawn1'].createCreep(standardBody, undefined, {role: 'harvester'});
        ...
        if (builders.length < 4 && Game.spawns['Spawn1'].canCreateCreep(standardBody) == OK) {
            var spawnedCreep = Game.spawns['Spawn1'].createCreep(standardBody, undefined, {role: 'builder'});
            console.log('Spawning new builder: ' + spawnedCreep);
            return;
        }
        
        var upgraders = _.filter(Game.creeps, (creep) => creep.memory.role == 'upgrader');
        if (upgraders.length < 3 && Game.spawns['Spawn1'].canCreateCreep(standardBody) == OK) {
            var spawnedCreep = Game.spawns['Spawn1'].createCreep(standardBody, undefined, {role: 'upgrader'});
            console.log('Spawning new upgrader: ' + spawnedCreep);
            return;
        }        
    }
};
